...
data$rank = factor(data$rank) #because they arent continuous values, rank and admit are converted into factor
...
data=mtcars
set.seed(123)
train=sample(1:nrow(data),0.7*nrow(data),replace=F,prob=NULL)
train
test=data[-train,]
test
cbind(train, test)
train_test=cbind(train,test)
head(train_test)
head(mtcars)
require(caTools)   #inorder to know not all zeros are in test data or train data, but equal amount of them present in both test and train data
set.seed(101)
