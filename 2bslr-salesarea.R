...
plot(density(residuals(fit1)))   #assumption: residuals must be in normal dist.
...
(Y = coef(fit1)[1] + coef(fit1)[2] * 4)  #instead of writing absolute values, we can write coef vector position 1 and position 2 for coef.vector in the place of intercept and x coefficient 

#using model
range(df$X)
(new1 = data.frame(X=c(4,5,3,2))) #name of col. should be same as in original dataset->X
